...
public class PlayerActivity extends AppCompatActivity
  ...
  private static final String META_DATA_FILE_NAME = "meta_data_";
  private static final String VIDEO_PROPERTIES = "video_properties_";
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    handler = new Handler();
    customAnalytics = new CustomAnalytics(getApplicationContext(), batteryManager, handler);

    try {
      handler.post(customAnalytics); // Start logging power consumption
    } catch (Exception e) {
      Log.e("PowerConsumptionLogger", "Error starting power consumption logger", e);
    }
  ...
  if (batteryManager != null) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      long energyCounter = batteryManager.getLongProperty(BatteryManager.BATTERY_PROPERTY_ENERGY_COUNTER)  ; //Năng lượng còn lại trong pin
      int capacity = batteryManager.getIntProperty(BatteryManager.BATTERY_PROPERTY_CAPACITY); //Dung lượng pin còn lại theo %
      long currentNow = batteryManager.getLongProperty(BatteryManager.BATTERY_PROPERTY_CURRENT_NOW); //Dòng điện hiện tại pin đang cung cấp hoặc nhận. Giá trị âm tức là đang sạc (nhận)
      long currentAverage = batteryManager.getLongProperty(BatteryManager.BATTERY_PROPERTY_CURRENT_AVERAGE); //Dòng điện trung bình pin đã cung cấp trong 1 time
      long chargeCounter = batteryManager.getLongProperty(BatteryManager.BATTERY_PROPERTY_CHARGE_COUNTER) / 1000000; //Dung lượng còn lại của pin
...
//    PowerConsumptionLogger.PowerConsumptionLogger(dataOutput);
  }
...
//    savePlaybackData();
  ...
  protected boolean initializePlayer() {
    if (player == null) {
      ...
      player.addAnalyticsListener(customAnalytics);
