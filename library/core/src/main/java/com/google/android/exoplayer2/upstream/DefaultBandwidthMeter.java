  ...
  public synchronized void onTransferEnd(DataSource source, DataSpec dataSpec, boolean isNetwork) {
    ...
    if (sampleElapsedTimeMs > 0) {
      ...
      if(batteryManager != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP){
        currentPowerConsumption = batteryManager.getLongProperty(BatteryManager.BATTERY_PROPERTY_CURRENT_NOW);
        //Dòng điện hiện tại pin đang cung cấp hoặc nhận. Giá trị âm tức là đang sạc (nhận)
      }
